...
public class UMath : Singleton<UMath> {
  ...
  public static void Start(){
    ...
    tempChild = (new GameObject("UMath_TempChild")).transform;
    tempParent = (new GameObject("UMath_TempParent")).transform;
  ...
  dir = UMath.SetVectorLength(dir, dir.magnitude * scale);
  ...
  // My crappy math functions
  public static bool IsVectorInSphere(Vector3 point, Vector3 spherePoint, float radius) {
    Vector3 relativePoint = (point - spherePoint).Abs();
    Vector3 slope = relativePoint.normalized;
    Vector3 max = slope * radius;
    if(relativePoint.x > max.x) return false;
    if(relativePoint.y > max.y) return false;
    if(relativePoint.z > max.z) return false;
    return true;
  }
  ...
  // 2D
  // 1D
  public static float Clamp(float value, float floor, float ceil) {
    return Mathf.Min(Mathf.Max(value, floor), ceil);
  }
  public static int Clamp(int value, int floor, int ceil) {
    return Mathf.Min(Mathf.Max(value, floor), ceil);
  }
}