...
//   void Start() {
//     this.GetDispatcher().On("model", delegate(UData _){ this.Rebuild() });
//   }
...
//     UData voxelData = model.GetChild("voxelsData");
//     UData chunksData = model.GetChild("chunksData");
...
//         if(!chunksData.GetChild(chunkX).Has(chunkY)) { continue; }
//         for(int z = zMin; z <= zMax; z+=CHUNK_SIZE) {
//           int chunkZ = z-(z%CHUNK_SIZE);
//           if(!chunksData.GetChild(chunkX).GetChild(chunkY).Has(chunkZ)) { continue; }
...
//           this.GetDispatcher.Once("rebuild", delegate(UData _) {
