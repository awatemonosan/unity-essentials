...
public class InputWrapper {
  protected UModel defaultPayload = new UModel();
  ...
  public void Trigger() {
    ...
    UModel payload = new UModel(defaultPayload);
    ...
    if(this.isBinary){
      ...
      if(oldStateBinary != stateBinary) {
        if(stateBinary == 0) {
          UInput.GetDispatcher().Trigger(this.inputName + ".released", payload);
        } else {
          UInput.GetDispatcher().Trigger(this.inputName + ".pressed", payload);
        }
      } else {
        if(stateBinary == 1) {
          UInput.GetDispatcher().Trigger(this.inputName + ".held", payload);
        }
      }
    } else {
      if(this.Delta() != 0) {
        UInput.GetDispatcher().Trigger(this.inputName + ".changed", payload);
      }
    }
  }
