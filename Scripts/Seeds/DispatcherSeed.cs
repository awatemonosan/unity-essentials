...
public class DispatcherSeed : MonoBehaviourExtended {
  ...
  void Awake() {Initialize();BroadcastService.Trigger("awake");}
  void Start() {BroadcastService.Trigger("start");}
  void FixedUpdate() {BroadcastService.Trigger("fixed_update");}
  void LateUpdate() {BroadcastService.Trigger("late_update");}
  void OnConnectedToServer() {BroadcastService.Trigger("connected_to_server");}
  void OnDestroy() {BroadcastService.Trigger("destroy");}
  void OnDisconnectedFromServer() {BroadcastService.Trigger("disconnected_from_server");}
  void OnFailedToConnect() {BroadcastService.Trigger("failed_to_connect");}
  void OnFailedToConnectToMasterServer() {BroadcastService.Trigger("failed_to_connect_to_master_server");}
  void OnPlayerConnected() {BroadcastService.Trigger("player_connected");}
  void OnPlayerDisconnected() {BroadcastService.Trigger("player_disconnected");}
  void OnPostRender() {BroadcastService.Trigger("post_render");}
  void OnPreCull() {BroadcastService.Trigger("pre_cull");}
  void OnPreRender() {BroadcastService.Trigger("pre_render");}
  void OnRenderObject() {BroadcastService.Trigger("render_object");}
  void OnServerInitialized() {BroadcastService.Trigger("server_initialized");}
  void Update() {BroadcastService.Trigger("update");}
  
  // Photon hooks
  // void OnConnectedToPhoton () { BroadcastService.Trigger("photon.connected"); }
  // void OnLeftRoom () { BroadcastService.Trigger("photon.left_room"); }
  ...
  void OnGUI() {
    ...
    BroadcastService.Trigger("gui");
    do {
      BroadcastService.Trigger("gui_"+e.type);
  ...
  void OnRenderImage(RenderTexture src, RenderTexture dest) {
    ...
    BroadcastService.Trigger("OnRenderImage", payload);
  }
...
////INPUTS/////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  private void Differ(string key, float newState, string type, bool isDelta) {
    ...
    if( oldState != newState || (isDelta && oldState != deltaOldState) ) {
      ...
      payload["event"]    = GenerateName( new string[]{"input"} );

      BroadcastService.Trigger(payload);
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Put type specific events down here
      if(type == "Keyboard" || type == "Joystick_Button"){
        string stateName;
        ...
        payload["event"] = GenerateName( new string[]{"input", type} );
        BroadcastService.Trigger(payload);
        payload["event"] = GenerateName( new string[]{"input", type, stateName} );
        BroadcastService.Trigger(payload);
        payload["event"] = GenerateName( new string[]{"input", type, key, stateName} );
        BroadcastService.Trigger(payload);
      } else if(type == "Mouse") {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  private void UpdateInputs(Hashtable payload) {
    ...
    foreach(KeyCode joyCode in joyCodes) {
      ...
      Differ(button, state, "Joystick_Button");
    }
    for(int j=0; j<=11; j++){
      for(int a=0; a<=19; a++){
